...
/**
 * Loads tracking vendors ONLY when:
 * - NEXT_PUBLIC_ENABLE_TRACKING === "true", AND
 * - the relevant ID exists.
 * Never hardcode IDs. Nothing here can crash when IDs are missing.
 *
 * Consent note: tracking scripts should respect the cookie banner state.
 * The simplest pattern is Google Consent Mode v2 — initialise consent as
 * "denied" here, then update it when the user accepts cookies.
 */
export function TrackingScripts() {
  if (!tracking.enabled) return null;

  const { gtmId, ga4Id, googleAdsId, metaPixelId, tiktokPixelId } = tracking;
...
fbq('track', 'PageView');`}
      ...
      {/* TikTok Pixel (placeholder integration) */}
      {tiktokPixelId ? (
